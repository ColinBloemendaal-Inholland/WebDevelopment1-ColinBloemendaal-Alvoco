    ...
    <main class="flex-grow-1 p-4">
        <section class="container-fluid m-0 py-5">
            <div class="row">
                <div class="col-12">
                    <header>
                        <h2 class="h4 mb-4">Bewerk Speler</h2>
                    </header>
                    <article class="card shadow-sm">
                        <div class="card-body">
                            <?php \View::partial('Layout.errors'); ?>
                            <form method="POST" action="/admin/spelers/<?= $data['speler']['id'] ?>">
                                            ...
                                            <?php foreach ($data['leden'] as $lid): ?>
                                                <option value="<?= e($lid['id']) ?>"
                                                    <?= (isset($_SESSION['form_old']['Leden_id']) && $_SESSION['form_old']['Leden_id'] == $lid['id'])
                                                        ? 'selected'
                                                        : ($data['speler']['Leden_id'] == $lid['id'] ? 'selected' : '') ?>>
                                                    <?= e($lid['fullname']) ?>
                                                </option>
                                            <?php endforeach; ?>
                                        </select>
                                            ...
                                            <?php foreach ($data['teams'] as $team): ?>
                                                <option value="<?= e($team['id']) ?>"
                                                    <?= (isset($_SESSION['form_old']['team_id']) && $_SESSION['form_old']['team_id'] == $team['id'])
                                                        ? 'selected'
                                                        : ($data['speler']['team_id'] == $team['id'] ? 'selected' : '') ?>>
                                                    <?= e($team['name']) ?>
                                                </option>
                                            <?php endforeach; ?>
                                        </select>
...
<script src="/JS/Admin/Spelers/edit.js"></script>
